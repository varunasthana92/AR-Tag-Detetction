...
def tagMat(img):
	...
	return gridImg[2:6,2:6], img


def tagID(img):
	grid, tag_warped = tagMat(img)
	# tl ,tr, br, bl = 0, 0, 0, 0
	tl, tr, br, bl = grid[0,0], grid[0,3], grid[3,3], grid[3,0]
	id_ = np.array([grid[2,1], grid[2,2], grid[1,2], grid[1,1]])
	angle = 0
	if(br):
		pass
	elif(tr):
...
while (cap.isOpened()):	
	...
	# contours, hierarchy = contoursFromCanny(gray)
	# 
	allChildsID = []
	allChilds = []
	allHier = []
	...
	""" for a given tag i in seq_tags, h and w are extracted as below
	for k in range(4):	# TL, TR, BR, BL
		h = seq_tags[i][k][0]
		w = seq_tags[i][k][1]   """
	...
	for H in H_allTags:
		...
		angle, tag_ID, tag_warped = tagID(im_out)
		cv2.putText(frame, str(tag_ID), (seq_tags[i][0][0]-20,seq_tags[i][0][1]-20), fontFace = cv2.FONT_ITALIC, thickness=5, fontScale = 1.5, color= (0,0,255))
		
	countour_image = cv2.drawContours(frame.copy(), all_tags, -1, (0, 255, 0), 3)
	cv2.imshow("counters", cv2.resize(countour_image, (192*5,108*5)))
